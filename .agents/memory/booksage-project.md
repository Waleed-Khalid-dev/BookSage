...
### Session 2026-09-09 Notes (Suggested Follow-Ups, Universal Citations, & Pinned AI Insights)
- **Suggested Follow-Up Questions:**
  - Implemented structured `@@FOLLOWUP:` protocol generated by AI in system prompts and parsed in `chatStore.ts`.
  - Rendered 3 interactive suggested question pills under AI responses in both `CopilotSidebar` and `AIChatView`.
  - Clicking any pill sends or pre-fills the question immediately.
- **Universal Citations & Page Jumps:**
  - Implemented universal subdivision and TOC title matcher in `CitationChip.tsx` supporting `[Law X]`, `[Ch. X]`, and `cite:X` across any book structure.
  - Aligned Python RAG context injection with exact TOC chapter indexes and page spans.
  - Fixed React child object rendering crash in message footer action buttons.
  - Enhanced `booksage-jump-page` custom event to accept both raw numbers and `{ pageNum: N }` across Single, Spread, and Continuous reader modes.
- **Building Era License Protection:**
  - Updated repository license to strict All Rights Reserved / Source-Available during the building era.
- **Conversation Branching & Pinned AI Insights:**
  - Persisted pinned insights in SQLite `chapters.ai_insights` with UUID and numeric chapter fallback.
  - Added interactive `📌 Pin` action button to `CopilotSidebar` and `AIChatView` with 2-second `✓ Pinned` feedback.
  - Added **PINNED INSIGHTS ({count})** section in BookReader left panel's Notes tab with chapter badges, jump-to-page click handlers, and unpin trash buttons.
  - Added collapsible **📌 Pinned AI Insights** section in `NotesViewer.tsx` displaying active chapter pins with copy and unpin actions.
  - Integrated pinned insights into markdown annotations export (`copyExportToClipboard` and `saveExportToFile`).
  - Added Zustand `insightsRefreshCounter` to synchronize pins across all open tabs and views.
- **Build & Verification:**
