...
- [ ] After every AI response, show 3 suggested follow-up questions as clickable pills
- [ ] Questions are generated by the AI alongside its main response (structured output)
- [ ] Clicking a suggestion sends it immediately as the next message

### Conversation Branching / Pinned Insights
*Inspired by: Readwise AI-Enhanced Annotations*

- [ ] "Pin" any AI response to a special "Insights" collection
- [ ] Pinned insights visible in the NotesViewer alongside chapter content
- [ ] Pinned to `chapters.ai_insights TEXT` SQLite column (new migration)

### Quick-Access Toolbar Presets
*Inspired by: Notion AI, Grammarly*

- [ ] Preset prompt buttons in sidebar header (one-click):
...
- [ ] Uses browser `SpeechRecognition` API (no backend needed)
- [ ] Transcribed text fills the input field (user reviews before sending)
- [ ] `Alt+M` keyboard shortcut to toggle microphone
...
- [ ] "Export" button in AIChatView and sidebar
- [ ] Generates a Markdown file with: book title, date, full conversation thread
- [ ] Saves to user-chosen folder via Tauri `dialog.save()`
...
- [ ] New `chat_sessions` SQLite table:
  ```sql
  ...
  ```
- [ ] Sessions listed in AIChatView left panel
- [ ] Auto-title: first 6 words of user's opening message
...
- [ ] Persistent floating circular button (`✦`) in the bottom-right of ANY view
- [ ] Clicking opens the CopilotSidebar regardless of current view
- [ ] Bouncy entrance animation, glow pulse idle state
- [ ] Draggable to any screen corner

### Persona / Tone Selector
*Inspired by: Character.ai, Claude tone options*

- [ ] Dropdown in sidebar/popup: "Copilot Persona"
  - [ ] 🎓 **Scholar** — academic, detailed, cites principles
  - [ ] 👨‍🏫 **Teacher** — explains simply, uses analogies
  - [ ] 🔥 **Coach** — motivational, action-oriented
  - [ ] 🤔 **Devil's Advocate** — challenges assumptions
- [ ] Persona is injected as a system prompt prefix
